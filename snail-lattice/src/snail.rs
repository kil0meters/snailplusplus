use crate::{utils::{Vec2, discrete_lerp}, maze::SNAIL_MOVEMENT_TIME};
...
impl Snail {
            ...
            for x in 0..SNAIL_IMAGE_SIZE {
                ...
                if snail_image[snail_px + 3] != 0 {
                    ...
                    let px = match self.direction {
                        Direction::Up => 4 * (((by + (SNAIL_IMAGE_SIZE - y)) as i32 + offset_y) as usize * buffer_width + bx + x + self.pos.x * 10 + 2),
                        Direction::Down => 4 * (((by + y + 2) as i32 + offset_y) as usize * buffer_width + bx + (SNAIL_IMAGE_SIZE - x) + self.pos.x * 10),
                        Direction::Left => 4 * ((by + x + self.pos.y * 10 + 2) * buffer_width + ((bx + (SNAIL_IMAGE_SIZE - y)) as i32 + offset_x) as usize),
                        Direction::Right => 4 * ((by + x + self.pos.y * 10 + 2) * buffer_width + ((bx + y + 2) as i32 + offset_x) as usize),
                    };
    ...
    pub fn move_forward(&mut self, maze: &Maze) -> bool {
        ...
        match self.direction {
            Direction::Up => if !maze.walls[coord] {
                self.pos.y -= 1;
                return true;
            },
            Direction::Down => if !maze.walls[coord + 1] {
                self.pos.y += 1;
                return true;
            },
            Direction::Left => if !maze.walls[coord + 2] {
                self.pos.x -= 1;
                return true;
            },
            Direction::Right => if !maze.walls[coord + 3] {
                self.pos.x += 1;
                return true;
            }
        }
